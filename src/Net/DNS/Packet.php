...
class Packet
{
    ...
    public $answersize;
    /**
     * An array of DNS\Question objects
     *
     * Contains all of the questions within the packet. Each question is
     * stored as an object of type  DNS\Question.
     *
     * @var array $question
     */
    public $question;
     ...
     * stored as an object of type DNS\RR.
     *
     * @var array $answer
     */
    public $answer;
    ...
    public $authority;
    /**
     * An array of ADDITIONAL objects
     *
    ...
    public function buildQuestion($name, $type = 'A', $class = 'IN')
    {
        ...
        $this->question[0] = new Question($name, $type, $class);
        $this->answer = null;
        $this->authority = null;
     ...
     * @param integer $offset // The location offset in the packet to where the label will be stored.
     *
     * @return string $compname // A binary string containing the compressed label.
     */
    public function dn_comp($name, $offset)
    {
        $names = explode('.', $name);
        $compname = '';
        while (count($names)) {
     ...
     * @param   integer $offset The location offset in the packet of the label to decompress.
     *
     ...
     */
    public static function dn_expand($packet, $offset)
    {
        ...
        while (1) {
            ...
            if ($len == 0) {
                ...
                $ptr = $ptr['i'];
                $ptr = $ptr & 0x3fff;
                $name2 = self::dn_expand($packet, $ptr);
     ...
     *
     * @param   string  $packet The packet data
     * @param   integer $offset The location offset in the packet of the label to extract.
     *
     * @return  array   Returns a list of type array($name, $offset) where
     *                  $name is the name of the label which was decompressed
     *                  and $offset is the offset of the next field in the
     *                  packet.  Returns array(null, null) on error
     */
    public static function label_extract($packet, $offset)
    {
        $packetlen = strlen($packet);
        $name = '';
        if ($packetlen < ($offset + 1)) {
     ...
     *                  needs to be parsed.
     */
    public function parse_question($data, $offset)
    {
        list($qname, $offset) = self::dn_expand($data, $offset);
        if (is_null($qname)) {
            return array(null, null);
        }
        ...
        $qtype = DNS::typesbyval($qtype);
        $qclass = DNS::classesbyval($qclass);

        $q = new Question($qname, $qtype, $qclass);
     ...
     *
     * @param string    $data   The packet data returned from the server
     * @param integer   $offset The location offset of the start of the resource record section.
     *
     * @return  array   An array of type array($rr, $offset) where $rr
     *                  is a DNS\RR object and $offset is the
     *                  location of the next section of the packet which
     *                  needs to be parsed.
     */
    public function parse_rr($data, $offset)
    {
        list($name, $offset) = self::dn_expand($data, $offset);
        if ($name === null) {
            return array(null, null);
        }
        ...
        $type = DNS::typesbyval($type);
        $class = DNS::classesbyval($class);
